...
export function loadProgress(): Progress {
  if (typeof window === 'undefined' || !window.localStorage) return { levelIndex: 0, completedIds: [] };
  try {
    const raw = window.localStorage.getItem(STORAGE_KEY);
    if (!raw) return { levelIndex: 0, completedIds: [] };
    ...
    return { levelIndex: 0, completedIds: [] };
  }
}
